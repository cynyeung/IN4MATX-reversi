...
socket.on('game_update', (payload) => {
    ...
    if(my_color === 'light') {
        $('#my_color').html('<h3 id="my_color">You are playing the light token</h3>');
    } else if(my_color === 'dark') {
        $('#my_color').html('<h3 id="my_color">You are playing the dark token</h3>');
    } else {
        $('#my_color').html('<h3 id="my_color">Error: Unknown color token for play</h3>');
    }

    if(payload.game.whose_turn === 'light') {
        $('#my_color').append('<h4> It is light token\'s turn.</h4>');
    } else if(payload.game.whose_turn === 'dark') {
        $('#my_color').append('<h4> It is dark token\'s turn.</h4>');
    } else {
    ...
    for(let row = 0; row < 8; row++) {
        for(let column = 0; column < 8; column++) {
            ...
            // Check to see if the server chnaged any spaces on the board
            if(old_board[row][column] !== board[row][column]) {
                ...
                if((old_board[row][column] === '?') && (board[row][column] === ' ')) {
                    ...
                    graphic = "empty-to-light.gif";
                    altTag = "light token";
                } else if((old_board[row][column] === '?') && (board[row][column] === 'd')) {
                    graphic = "empty-to-dark.gif";
                    altTag = "dark token";
                } else if((old_board[row][column] === ' ') && (board[row][column] === 'l')) {
                    graphic = "empty-to-light.gif";
                    altTag = "light token";
